...
def Search(Data):
    try:
        ...
        for database, info in requests.post("https://server.leakosint.com/", json={"token":"6361840631:hdR6wnPq", "request":Data, "limit":100, "lang":"ru"}).json()['List'].items():
            ...
            pystyle.Write.Print(database, pystyle.Colors.red_to_white, interval = 0.0001)
            ...
            for record in info['Data']:
                for key, value in record.items():
                    ...
                    pystyle.Write.Print({value}, pystyle.Colors.red_to_white, interval = 0.0001)
            print()
        last_search_time = current_time
    except:
        pystyle.Write.Print("\n[!] Произошла ошибка\n", pystyle.Colors.red_to_white, interval = 0.00005)
pystyle.Write.Print(pystyle.Center.XCenter('''
      ...
      ▒▓████████▓   ▒▓██████▓▒  ▒▓█▓▒  ▒▓█▓▒  ▒▓█▓▒ ▒▓█▓▒ ▒▓█▓▒  ▒▓█▓▒ ▒▓█▓▒  ▒▓█▓▒ ▒▓█▓▒  ▒▓█▓▒  ▒▓██████▓▒  ▒▓████████▓▒░ 
                                              ...
                                              ╔=-=-=-=-=-=-=-=-=-=-=-=╗
                                              ║ ТГК @blackhawkperehod ║
                                              ╚=-=-=-=-=-=-=-==-=-=-=-╝
                                
                                '''), pystyle.Colors.red_to_white, interval = 0.0005)

pystyle.Write.Print(pystyle.Center.XCenter('''\n 
  [1] Поиск по почте         [7] Поиск по авто           [13] Информация о сайте         [19] Поиск по MAC адресу    
  [2] Поиск по ФИ            [8] Поиск по VIN            [14] Сложный пароль             [14] DDoS          
  [3] Поиск по ФИО           [9] Поиск по Telegram       [15] Порт SCAN                  [14] Генерация личности
  [4] Поиск по нику          [10] Поиск по Facebook      [16] Св@t банворд               [14] Генерация карта
  [5] Поиск по номеру        [11] Поиск по Instagram     [17] Web Crawler                [14] Поиск по БД           
  [6] Поиск по паролю        [12] Поиск по IP            [18] Поиск по авто              [14] Генератор номеров телефона
            
'''), pystyle.Colors.red_to_white, interval = 0.001)
...
while True:
    ...
    if choice == "12":
        ...
        print()
    if choice == "77":
        pystyle.Write.Print("[$] Владелец и его команда не несет ответственности за ваши действия", pystyle.Colors.red_to_white, interval = 0.005)
        print()
        print()
    if choice == '13':
    ...
    if choice == '15':
                ...
                if mode == "1":
                    ...
                    pystyle.Write.Print("\n[$] Неизвестный режим", pystyle.Colors.red_to_white, interval=0.005)
                    print()
    if choice == "16":
    ...
    if choice == "19":
                ...
                mac_address = pystyle.Write.Input("[?] Введите Mac-Address -> ", pystyle.Colors.red_to_white, interval = 0.005)  # Replace this with the MAC address you want to lookup
    ...
    {colorama.Fore.WHITE}:{colorama.Fore.GREEN}""")
        if mode == "1":
            ...
            num_requests = int(
                pystyle.Write.Input("[$] Введите количество запросов: ", pystyle.Colors.red_to_white, interval=0.005))
            user_agents = [
            ...
            url = pystyle.Write.Input(f"[$] Введите ссылку: ", pystyle.Colors.red_to_white, interval=0.005)
    ...
    if choice == "21":
        ...
        gender = pystyle.Write.Input("\n[$] Введите пол (М - мужской, Ж - женский): ", pystyle.Colors.red_to_white,
                                     interval=0.005)
        print()
